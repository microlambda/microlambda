...
export const waitUntilCertificateIssued = async (
  ...
  logger?.debug(serviceName, 'DescribeCertificateOutput', JSON.stringify(details, null, 2));
  ...
  const throwError = (): void => {
    logger?.error('Cannot activate certificate. Related hosted zone not found on Route53');
    logger?.error('Ask your domain administrator to create the following CNAME record and re-run deployment');
    logger?.error(record);
  ...
  logger?.info('Waiting for the certificate to be active. Please wait this can take up to 30 minutes');
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    setTimeout(() => {
      ...
      logger?.error('Please double-check that the correct activation record have been created');
