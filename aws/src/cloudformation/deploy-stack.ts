...
export const deployStack = async (params: {
  region: string,
  stackName: string,
  templateBody: string,
  ...
  if (await stackExists({ region, stackName })) {
    ...
    await waitForStackStatus({ region, stackName, status: 'UPDATE_COMPLETE', failOn: ['CREATE_FAILED', 'ROLLBACK_COMPLETE', 'ROLLBACK_FAILED']}, logger);
  } else {
    ...
    await waitForStackStatus({ region, stackName, status: 'CREATE_COMPLETE', failOn: ['UPDATE_FAILED', 'ROLLBACK_FAILED', 'ROLLBACK_COMPLETE', 'UPDATE_ROLLBACK_FAILED', 'UPDATE_ROLLBACK_COMPLETE']}, logger);
  }
}
