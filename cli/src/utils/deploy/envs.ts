...
export class EnvsResolver {
  ...
  async resolve(region: string): Promise<Map<string, Record<string, string>>> {
    const alreadyResolved = this._envs.get(region);
    if (alreadyResolved) {
      return alreadyResolved;
    }
    const env = await resolveEnvs(this.project, this.env, SSMResolverMode.ERROR, region, this._logger);
