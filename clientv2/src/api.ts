...
import type { IEventLog, INodeSummary, IServiceLogs, LogLevel, SchedulerStatus } from '@microlambda/types';
...
async function _doActionOnService(service: string, action: string): Promise<void> {
...
export async function fetchServiceLogs(service: string): Promise<IServiceLogs> {
  const response = await fetch(`${env.apiUrl}/api/services/${encodeURIComponent(service)}/logs`);
...
export async function fetchCompilationLogs(node: string): Promise<string[]> {
  const response = await fetch(`${env.apiUrl}/api/nodes/${encodeURIComponent(node)}/tsc/logs`);
  const compilationLogs = await response.json();
  log.info('Build Logs updated', node, compilationLogs.length);
  return compilationLogs;
}

export async function fetchEventLogs(level: LogLevel = 'info'): Promise<IEventLog[]> {
