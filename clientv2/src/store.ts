...
import { fetchCompilationLogs, fetchEventLogs, fetchGraph, fetchSchedulerStatus, fetchServiceLogs } from './api';
import type { IEventLog, INodeSummary, IServiceLogs, SchedulerStatus } from '@microlambda/types';
import { Debouncer } from './utils/debouncer';
import { logger } from './logger';

const log = logger.scope('(store)');
log.info('Connecting websocket on', env.apiUrl);
let socket = io(env.apiUrl);
...
function createGraph() {
	const { subscribe, set } = writable<INodeSummary[]>([], (set) => {
...
function createServiceLogs() {
  const { subscribe, set, update } = writable<IServiceLogs>({
    offline: [],
    deploy: [],
    createDomain: [],
  });
  return { subscribe, set, update, fetch: async (service: string) => {
      set(await fetchServiceLogs(service));
    }
  }
}
function createCompilationLogs() {
  const { subscribe, set, update } = writable<string[]>([]);
  return { subscribe, set, update, fetch: async (node: string) => {
      set(await fetchCompilationLogs(node));
    }
  }
}

function createSchedulerStatus() {
  const { subscribe, set } = writable<SchedulerStatus | null>(null, (set) => {
...
export const services = derived(graph, $graph => $graph.filter(n => n.port));
export const packages = derived(graph, $graph => $graph.filter(n => !n.port));
