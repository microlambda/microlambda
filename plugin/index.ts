...
class ServerlessMilaOffline {
    ...
    private _log = {
      ...
      error: (...args: unknown[]) => {
        this.serverless.cli.log(chalk.red(args));
      }
    }
    ...
    private async _beforeOffline() {
      await this._getDependenciesGraph();
      this._resolveCurrentService();
      this._resolveOutDir();
      await this._transpile();
      this._assign(this.serverless, 'service.custom.serverless-offline.location', relative(process.cwd(), this._outDir || 'lib'));
      this._watch();
    }

    private _assign = (obj: {[key: string]: any}, path: string, value: any) => {
    ...
    private async _getDependenciesGraph() {
    ...
    private _resolveCurrentService() {
      this._log.debug(`cwd: ${process.cwd()}`);
      this._service = this._graph?.getNodes().find((s) => s.getLocation() === process.cwd()) as Service;
      if (!this._service) {
    ...
    private _resolveOutDir() {
      ...
      this._log.info(`Transpiling ${this._service?.getName()} to ${this._outDir}`);
    }
