...
import { ApiType, ILogger } from "../../types";
...
export const getApiId = async (
  ...
  let resource;
  try {
    const response = await getStackResource(
      region,
      logicalResourceId,
      stackName,
      logger
    ...
    logger?.debug(
      serviceName,
      "Something wrong happened, maybe the resource if in a nested stack"
    );
    resource = await getNestedStackResource(
      region,
      logicalResourceId,
      stackName,
      logger
  ...
  if (!resource) {
    throw new Error(
      `Failed to find stack API Gateway resource ${logicalResourceId} for ${stackName}\n`
    );
  }
  const apiId = resource.PhysicalResourceId;
  if (!apiId) {
    throw new Error(
      `No ApiId associated with CloudFormation stack ${stackName}`
    );
  }
  return apiId;
};
