...
export const beforeDeploy = async (
  ...
  const customDomain = await aws.apiGateway.getCustomDomain(
    region,
    domain,
    logger,
  );
  if (!customDomain) {
    ...
    let certificate = await aws.certificateManager.getClosestCertificate(
      region,
      domain,
      logger,
    );
    ...
    if (!certificate) {
      ...
      certificate = await aws.certificateManager.createCertificate(
        region,
        domain,
        logger,
      );
  ...
  // create DNS record
  await aws.route53.createLatencyRecord(
    region,
    domain,
    `${serviceName}-${region}`,
    logger,
