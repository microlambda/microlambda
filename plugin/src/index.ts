...
class ServerlessMicrolambdaPlugin {
    ...
    this.hooks = {
      ...
      'before:package:createDeploymentArtifacts': this._plugHook(async () => {
        ...
        if (this._pluginConfig?.packagr?.useLayer) {
          this.serverless.service.provider.environment.NODE_PATH =
            './:/opt/node_modules';
        }
  ...
  private _resolveBasicInformation(): {
    region: string;
    stage: string;
    stackName: string;
  } {
  ...
  private async _getDependenciesGraph(): Promise<void> {
    if (!this._graph) {
      ...
      process.env.MILA_SERVICES_LENGTH = this._graph.services.size.toString();
    }
  }
  ...
  private async _resolveCurrentService(): Promise<void> {
    ...
    if (!this._service) {
      ...
      this._service = Array.from(this._graph.services.values()).find(
        (s) => s.root === normalizedCwd,
      );
      if (!this._service) {
