...
describe('Integration Tests', function () {
  ...
  describe('Domain Manager Is Enabled', function () {
    ...
    itParam('${value.testDescription}', testCases, async (value) => {
      ...
      const created = await utilities.createResources(
        value.testFolder,
        value.testDomain,
        RANDOM_STRING,
        true,
  ...
  describe('Basepath Mapping Is Empty', function () {
    // eslint-disable-line func-names
  ...
  describe('Basepath Mapping Is Set', function () {
    // eslint-disable-line func-names
    this.timeout(15 * 60 * 1000); // 15 minutes in milliseconds
  ...
  describe('Basepath Mapping Is Empty And Fix Works', function () {
    // eslint-disable-line func-names
    this.timeout(15 * 60 * 1000); // 15 minutes in milliseconds
    const testName = 'null-basepath-mapping';
    const testURL = `${testName}-${RANDOM_STRING}.${TEST_DOMAIN}`;
  ...
  describe('Create domain is idempotent', function () {
    // eslint-disable-line func-names
    this.timeout(15 * 60 * 1000); // 15 minutes in milliseconds
  ...
  describe('Deploy is idempotent', function () {
    // eslint-disable-line func-names
    this.timeout(15 * 60 * 1000); // 15 minutes in milliseconds
    ...
    it('Deploys multiple times without failure', async () => {
      ...
      await utilities.createTempDir(TEMP_DIR, testName);
      createDomainSuccess = await utilities.slsCreateDomain(
        TEMP_DIR,
        RANDOM_STRING,
      );
      await utilities.sleep(60);
  ...
  describe('Migrating from 2.x.x to 3.x.x works', function () {
    // eslint-disable-line func-names
    this.timeout(15 * 60 * 1000); // 15 minutes in milliseconds
    ...
    before(async () => {
      ...
      await utilities.exec(
        `mkdir -p ${TEMP_DIR} && cp -R test/integration-tests/${testName}/. ${TEMP_DIR}`,
      );
      await utilities.exec(
        `cd ${TEMP_DIR}/ && npm install serverless-domain-manager@2.6.13`,
      );
    });

    it('Creates a basepath mapping', async () => {
      await utilities.exec(
        `cd ${TEMP_DIR} && sls create_domain --RANDOM_STRING ${RANDOM_STRING}`,
      );
      await utilities.sleep(60);
      await utilities.exec(
        `cd ${TEMP_DIR} && sls deploy --RANDOM_STRING ${RANDOM_STRING}`,
      );
      await utilities.sleep(60);
      await utilities.exec(
        `cp -R . ${TEMP_DIR}/node_modules/serverless-domain-manager`,
      );
      await utilities.exec(
        `cd ${TEMP_DIR} && sls deploy --RANDOM_STRING ${RANDOM_STRING}`,
      );
    ...
    after(async () => {
      await utilities.exec(
        `cd ${TEMP_DIR} && sls remove --RANDOM_STRING ${RANDOM_STRING}`,
      );
      await utilities.sleep(60);
      await utilities.exec(
        `cd ${TEMP_DIR} && sls delete_domain --RANDOM_STRING ${RANDOM_STRING}`,
      );
      await utilities.sleep(60);
      await utilities.exec(`rm -rf ${TEMP_DIR}`);
    });
  ...
  describe('Migrating from 2.x.x to 3.x.x works given basepath', function () {
    // eslint-disable-line func-names
    this.timeout(15 * 60 * 1000); // 15 minutes in milliseconds
