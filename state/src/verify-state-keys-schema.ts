...
export const verifyStateKeysSchema = async (config: IRootConfig): Promise<boolean> => {
  ...
  const checkGSIs =
    ...
    keySchemaEquals(metadata.Table?.GlobalSecondaryIndexes.find((gsi) => gsi.IndexName === 'GS1')?.KeySchema, 'k2,k1') &&
    keySchemaEquals(metadata.Table?.GlobalSecondaryIndexes.find((gsi) => gsi.IndexName === 'GS2')?.KeySchema, 'k3,k2') &&
